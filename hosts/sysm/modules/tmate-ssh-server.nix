# Copied from nixpkgs and adapted
# Why? because networking.firewall.allowedTCPPorts, networking.domain are bringing in god into the imports
# (god i.e. all the nixos modules are not enough, nixpkgs systemd module /system/boot/systemd.nix conflicts with sysm's systemd module)
{
  config,
  lib,
  pkgs,
...
in
{
  options.services.tmate-ssh-server = {
    ...
    host = mkOption {
      ...
      example = lib.literalExpression "$(cat /etc/ngrok-tcp-hostname)";
    ...
    systemd.services.tmate-ssh-server = {
      ...
      serviceConfig = {
        ExecStart = pkgs.writeShellApplication {
          name = "tmate-ssh-server-script";
          text = ''
            ${cfg.package}/bin/tmate-ssh-server \
              -h ${cfg.host} \
              -p ${toString cfg.port} \
              -q ${toString cfg.advertisedPort} \
              -k ${keysDir}
          '';
        };
      };
      preStart = mkIf (cfg.keysDir == null) ''
        [[ ! -d ${defaultKeysDir} ]] && mkdir -p ${defaultKeysDir}
        [[ ! -f ${edKey} ]] && ${pkgs.openssh}/bin/ssh-keygen -t ed25519 -f ${edKey} -N ""
        [[ ! -f ${rsaKey} ]] && ${pkgs.openssh}/bin/ssh-keygen -t rsa -f ${rsaKey} -N ""
      '';
    };
  };
